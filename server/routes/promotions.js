...
router.post('/redeem', auth, redeemLimiter, async (req, res) => {
  try {
    ...
    res.status(400).json({ success: false, message: e.message || 'Failed to redeem code' });
  }
});
  ...
  try {
    ...
    res.json({
      ...
      data: {
        ...
        referredCount,
        shareMessage: `Join BetaKE and get a bonus on your first deposit! Use my code ${user.referralCode} when you sign up.`
      }
    });
  } catch (e) {
