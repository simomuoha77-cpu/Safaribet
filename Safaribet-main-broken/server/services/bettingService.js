...
/**
 * Combines Bet Builder legs into one price, applying a correlation discount.
 *
 * Naively multiplying odds from different markets on the SAME match overstates
 * the true combined price, because outcomes on one match aren't independent —
 * e.g. "Home Win" and "Over 2.5 Goals" are positively correlated (a team winning
 * comfortably often means more goals), so the real combined probability is HIGHER
 * than independent multiplication implies, meaning the fair combined ODDS should
 * be LOWER than naive multiplication gives.
 *
 * This applies a fixed, disclosed correlation discount per additional leg rather
 * than pretending to model exact market correlation (which would require data we
 * don't have — real correlation coefficients between markets). This is standard
 * practice among bookmakers offering "Bet Builder" products: apply a conservative
 * discount rather than pay out naive independent-odds pricing, which would be
 * mathematically generous to the bettor at the platform's expense.
 */
const BET_BUILDER_DISCOUNT_PER_EXTRA_LEG = 0.90; // each leg beyond the first discounts the running total by 10%

function calculateBetBuilderOdds(legs) {
  let combined = legs[0].odds;
  for (let i = 1; i < legs.length; i++) {
    combined = combined * legs[i].odds * BET_BUILDER_DISCOUNT_PER_EXTRA_LEG;
  }
