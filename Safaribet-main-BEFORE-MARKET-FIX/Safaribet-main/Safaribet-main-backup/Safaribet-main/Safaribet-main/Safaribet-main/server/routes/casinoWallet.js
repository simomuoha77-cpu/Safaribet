...
if (!SHARED_SECRET()) console.warn('[casino/webhook] CASINO_WEBHOOK_SECRET is not configured');
...
router.post('/debit', verifyWebhook, async (req, res) => {
  try {
    const { utoken, amount, roundId, gameId, reference, provider, currency } = req.body;
    ...
    if (currency && String(currency).toUpperCase() !== 'KES') {
    ...
    // Provider transaction/reference is the primary idempotency key. Fall back
    // to roundId only for older providers that do not send a transaction id.
    const providerTransactionId = String(reference || req.body.transactionId || `bet:${roundId}`);
    const ledgerRef = `casino_debit_${providerTransactionId}`;
    ...
    if (existing) {
      const bal = await walletService.getBalance(userId);
      console.log(`[CASINO_BET] duplicate userId=${userId} transactionId=${providerTransactionId}`);
      return res.json({
        success: true, userId, roundId,
        debited: Math.abs(existing.amount),
        balance: bal.spendable, newBalance: bal.spendable,
        currency: 'KES', duplicate: true
      });
    }
    ...
    const wallet = await walletService.debit(
      userId, 'main', debitAmount, 'casino_bet', ledgerRef,
      { gameId, roundId, provider, providerTransactionId, currency: 'KES' }
    );
    if (!wallet) {
    ...
    console.log(`[CASINO_BET] userId=${userId} gameId=${gameId || '-'} roundId=${roundId} transactionId=${providerTransactionId} amount=${debitAmount}`);

    res.json({
      success: true, userId, roundId, transactionId: tx.providerTransactionId,
      debited: debitAmount, balance: newBal.spendable, newBalance: newBal.spendable, currency: 'KES'
    });
  } catch(e) {
    // A duplicate request can race the first insert. Never charge again.
    if (e?.code === 11000) {
      const providerTransactionId = String(req.body.reference || req.body.transactionId || `bet:${req.body.roundId || ''}`);
      const existing = await Transaction.findOne({ userId: await resolveUserId(req.body.userId, req.body.utoken), providerTransactionId }).lean().catch(() => null);
      if (existing) {
        const bal = await walletService.getBalance(existing.userId);
        return res.json({ success: true, userId: existing.userId, balance: bal.spendable, newBalance: bal.spendable, duplicate: true });
      }
    }
...
router.post('/credit', verifyWebhook, async (req, res) => {
  try {
    const { userId: suppliedUserId, utoken, amount, roundId, gameId, reference, provider, currency } = req.body;
    const userId = await resolveUserId(suppliedUserId, utoken);
    if (!userId || !amount || !roundId) {
      return res.status(400).json({ success: false, message: 'userId or utoken, amount, roundId required' });
    }
    if (currency && String(currency).toUpperCase() !== 'KES') {
    ...
    const providerTransactionId = String(reference || req.body.transactionId || `win:${roundId}`);
    const ledgerRef = `casino_credit_${providerTransactionId}`;

    const existing = await Transaction.findOne({
      userId, type: 'casino_win', providerTransactionId
    }).lean();
    if (existing) {
      const bal = await walletService.getBalance(userId);
      console.log(`[CASINO_WIN] duplicate userId=${userId} transactionId=${providerTransactionId}`);
      return res.json({ success: true, userId, balance: bal.spendable, newBalance: bal.spendable, currency: 'KES', duplicate: true });
    }
    ...
    const wallet = await walletService.credit(
      userId, 'main', creditAmount, 'casino_win', ledgerRef,
      { gameId, roundId, provider, providerTransactionId, currency: 'KES' }
    );

    const tx = await Transaction.create({
      userId, type: 'casino_win', amount: creditAmount,
      balance: wallet.main,
      reference: ledgerRef,
      providerTransactionId,
      roundId: String(roundId),
    ...
    const newBal = await walletService.getBalance(userId);
    console.log(`[CASINO_WIN] userId=${userId} gameId=${gameId || '-'} roundId=${roundId} transactionId=${providerTransactionId} amount=${creditAmount}`);
    ...
    res.json({
      success: true, userId, roundId, transactionId: tx.providerTransactionId,
      credited: creditAmount, balance: newBal.spendable, newBalance: newBal.spendable, currency: 'KES'
    });
  } catch(e) {
    if (e?.code === 11000) {
      const providerTransactionId = String(req.body.reference || req.body.transactionId || `win:${req.body.roundId || ''}`);
      const userId = await resolveUserId(req.body.userId, req.body.utoken).catch(() => null);
      const existing = userId ? await Transaction.findOne({ userId, type: 'casino_win', providerTransactionId }).lean().catch(() => null) : null;
      if (existing) {
        const bal = await walletService.getBalance(existing.userId);
        return res.json({ success: true, userId: existing.userId, balance: bal.spendable, newBalance: bal.spendable, duplicate: true });
      }
...
router.post('/rollback', verifyWebhook, async (req, res) => {
  try {
    ...
    const providerTransactionId = String(reference || req.body.transactionId || `rollback:${roundId}`);
    const ledgerRef = `casino_rollback_${providerTransactionId}`;

    const existing = await Transaction.findOne({
      userId, type: 'casino_refund', providerTransactionId
    }).lean();
    if (existing) {
      const bal = await walletService.getBalance(userId);
      return res.json({ success: true, userId, balance: bal.spendable, newBalance: bal.spendable, currency: 'KES', duplicate: true });
    ...
    const debit = await Transaction.findOne({
      userId, type: 'casino_bet',
      $or: [
        { roundId: String(roundId) },
        { reference: new RegExp(`^casino_debit_.*${String(roundId).replace(/[.*+?^${}()|[\]\\]/g, '\\$&')}`) }
      ]
    }).lean();
    if (!debit) {
    ...
    const wallet = await walletService.credit(
      userId, 'main', refundAmount, 'casino_refund', ledgerRef,
      { gameId, roundId, provider, providerTransactionId, currency: 'KES' }
    );

    const tx = await Transaction.create({
      userId, type: 'casino_refund', amount: refundAmount,
      balance: wallet.main,
      reference: ledgerRef,
      providerTransactionId,
      roundId: String(roundId),
    ...
    const newBal = await walletService.getBalance(userId);
    console.log(`[CASINO_ROLLBACK] userId=${userId} gameId=${gameId || '-'} roundId=${roundId} transactionId=${providerTransactionId} amount=${refundAmount}`);

    res.json({
      success: true, userId, roundId, transactionId: tx.providerTransactionId,
      refunded: refundAmount, balance: newBal.spendable, newBalance: newBal.spendable, currency: 'KES'
    });
  } catch(e) {
