...
const JUAN_URL = () => String(process.env.JUANAI_URL || 'https://bitfreezeee.onrender.com').replace(/\/$/, '');
...
router.get('/juan-games', async (req, res) => {
  try {
    ...
    const games = Array.isArray(r.data?.data) ? r.data.data : Array.isArray(r.data?.games) ? r.data.games : Array.isArray(r.data?.data?.games) ? r.data.data.games : [];
